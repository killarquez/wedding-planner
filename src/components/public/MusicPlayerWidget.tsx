...
export const MusicPlayerWidget: React.FC<Props> = ({ lang: propLang, onReplayIntro }) => {
  const context = useLanguage();
  const lang = propLang || context?.lang || 'en';
  const pathname = usePathname();
  const isPublicPage = pathname === '/' || pathname.startsWith('/rsvp');
  ...
  // Pause playback if user navigates to an admin or login page
  useEffect(() => {
    if (!isPublicPage && isPlaying) {
      pauseYouTubePlayback();
    }
  }, [pathname, isPublicPage, isPlaying]);
  ...
  return (
    <>
      ...
      {/* Floating Interactive Music Disc Widget (Visible on public guest routes) */}
      {isPublicPage && (
