...
export function generateWeddingIcsFile(guestName?: string): string {
  ...
  // Dec 12, 2026 17:30:00 PST (UTC-8) -> 20261213T013000Z
  // End Dec 12, 2026 23:30:00 PST -> 20261213T073000Z
  const startTimeUtc = "20261213T013000Z";
  const endTimeUtc = "20261213T073000Z";
  const uid = `wedding-trang-alfredo-20261212-${Date.now()}@weddingplanner.local`;
...
export function downloadIcsFile(guestName?: string) {
  ...
  link.href = url;
  link.setAttribute('download', 'Trang_Alfredo_Wedding_Dec_12_2026.ics');
